...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
 ...
 pr_err("mas_ioctl TIMEOUT_WAKELOCK");
            break;
        case TAP_DOWN:
...
pr_err("mas_ioctl SET_MODE");
 ...
 pr_err("mas_ioctl GET_MODE");
        ...
        case ENABLE_IRQ:
	    init_interrupt();
 pr_err("mas_ioctl ENABLE_IRQ");
	    break;
	case DISABLE_IRQ:
	    deinit_interrupt();
 pr_err("mas_ioctl DISABLE_IRQ");
	    break;
        case MA_IOC_GVER:
            mutex_lock(&ioctl_lock);
            //*((unsigned int*)arg) = MA_DRV_VERSION;
            mutex_unlock(&ioctl_lock);
 pr_err("mas_ioctl MA_IOC_GVER");
            break;
		case MA_IOC_PWON:
 pr_err("mas_ioctl MA_IOC_PWON");
			mas_switch_power(1);
			break;
		case MA_IOC_PWOF:
 pr_err("mas_ioctl MA_IOC_PWOF");
			mas_switch_power(0);
			break;
        case SET_SPI_SPEED:
 pr_err("mas_ioctl SET_SPI_SPEED");
            ret = copy_from_user(&ma_speed, (unsigned int*)arg, sizeof(unsigned int));
	    /*delete by guq@microarray.com.cn*/
	    /*ma_spi_change(smas->spi, ma_speed, 0);*/
            break;
        case WAIT_FACTORY_CMD:
 pr_err("mas_ioctl WAIT_FACTORY_CMD");
            smas->u2_flag = 0;
           	ret = wait_event_freezable(U2_Waitq, smas->u2_flag != 0);
			break;
        case WAKEUP_FINGERPRINTD:
 pr_err("mas_ioctl AKEUP_FINGERPRINTD");
            smas->u2_flag = 1;
            wake_up(&U2_Waitq);
            break;
        case WAIT_FINGERPRINTD_RESPONSE:
 pr_err("mas_ioctl WAIT_FINGERPRINTD_RESPONSE");
            smas->u1_flag = 0;
 ...
 pr_err("mas_ioctl WAKEUP_FACTORY_TEST_SEND_FINGERPRINTD_RESPONSE");
            mutex_lock(&ioctl_lock);
            ret = copy_from_user(&ma_drv_reg, (unsigned int*)arg, sizeof(unsigned int));
            mutex_unlock(&ioctl_lock);
            msleep(4);
 ...
 pr_err("mas_ioctl WAIT_SCREEN_STATUS_CHANGE");
			screen_flag = 0;
 ...
 pr_err("mas_ioctl GET_INTERRUPT_STATUS");
...
int mas_probe(struct spi_device *spi) {
	int ret = 0;
	if (!strstr(saved_command_line, "androidboot.device=HD1")) goto err1;
