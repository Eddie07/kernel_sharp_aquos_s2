    ...
    u32 ecc_length)
{
    ...
    ret = fts_i2c_write_8719(client, cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < 100; i++) {
        msleep(1);
        ret = fts_i2c_read_8719(client, cmd, 1, val, 1);
        if (ret < 0) {
    ...
    ret = fts_i2c_read_8719(client, cmd, 1, val, 2);
    if (ret < 0) {
...
static int fts_pram_ecc_cal_xor(struct i2c_client *client)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_ROMBOOT_CMD_ECC, &reg_val);
    if (ret < 0) {
        FTS_ERROR("read pramboot ecc fail");
        return ret;
    }
...
static int fts_pram_write_buf(struct i2c_client *client, u8 *buf, u32 len)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_i2c_write_8719(client, packet_buf, packet_len + FTS_CMD_WRITE_LEN);
        if (ret < 0) {
...
static int fts_pram_start(struct i2c_client *client)
{
    ...
    ret = fts_i2c_write_8719(client, &cmd, 1);
    if (ret < 0) {
...
static int fts_pram_write_remap(struct i2c_client *client)
{
	...
	char pram_name[FTS_FW_NAME_LEN] = {0};
	struct device *dev = NULL;
	const struct firmware *pram = NULL;
	int i = 0;
    ...
    ret = fts_get_pram_name(pram_name,FTS_FW_NAME_LEN);
	if (ret) {
...
#endif
	ret = request_firmware(&pram, pram_name, dev);
	if (ret != 0) {
    ...
    pb_buf = kzalloc(pram->size * sizeof(u8), GFP_KERNEL);
    if (NULL == pb_buf) {
    ...
    pb_len = pram->size;
    for(i = 0; i < pram->size; i++) {
        pb_buf[i] = pram->data[i];
    }
    FTS_INFO("upgrade pram file len:%d", pb_len);
...
static int fts_pram_init(struct i2c_client *client)
{
    ...
    ret = fts_i2c_read_8719(client, wbuf, 1, &reg_val, 1);
    if (ret < 0) {
    ...
    wbuf[1] = reg_val;
    wbuf[2] = 0x00;
    ret = fts_i2c_write_8719(client, wbuf, 3);
    if (ret < 0) {
...
bool fts_fwupg_check_fw_valid(struct i2c_client *client)
{
    ...
    ret = fts_wait_tp_to_valid_8719(client);
    if (ret < 0) {
...
int fts_fwupg_get_boot_state(struct i2c_client *client, enum FW_STATUS *fw_sts)
{
    ...
    ret = fts_i2c_write_8719(client, cmd, 2);
    if (ret < 0) {
    ...
    ret = fts_i2c_read_8719(client, cmd, cmd_len, val, 2);
    if (ret < 0) {
...
int fts_fwupg_reset_to_boot(struct i2c_client *client)
{
    ...
    ret = fts_i2c_write_reg_8719(client, FTS_REG_UPGRADE, FTS_UPGRADE_AA);
    if (ret < 0) {
    ...
    ret = fts_i2c_write_reg_8719(client, FTS_REG_UPGRADE, FTS_UPGRADE_55);
    if (ret < 0) {
    ...
    int retries_delay)
{
    ...
    for (i = 0; i < retries; i++) {
        ...
        ret = fts_i2c_read_8719(client, &cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
...
int fts_fwupg_ecc_cal(struct i2c_client *client, u32 saddr, u32 len)
{
    ...
    ret = fts_i2c_write_8719(client, wbuf, 1);
    if (ret < 0) {
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_i2c_write_8719(client, wbuf, FTS_CMD_ECC_CAL_LEN);
        if (ret < 0) {
    ...
    ret = fts_i2c_read_8719(client, wbuf, 1, val, 1);
    if (ret < 0) {
...
int fts_flash_read_buf(struct i2c_client *client, u32 saddr, u8 *buf, u32 len)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_i2c_write_8719(client, wbuf, FTS_CMD_READ_LEN);
        if (ret < 0) {
        ...
        ret = fts_i2c_read_8719(client, NULL, 0, buf + offset, packet_len);
        if (ret < 0) {
...
static int fts_lic_get_vid_in_tp(struct i2c_client *client, u16 *vid)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_REG_VENDOR_ID, &val[0]);
    if (fts_data->ic_info.is_incell)
        ret = fts_i2c_read_reg_8719(client, FTS_REG_MODULE_ID, &val[1]);
    if (ret < 0) {
...
static int fts_lic_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_REG_LIC_VER, ver);
    if (ret < 0) {
...
static int fts_param_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_REG_IDE_PARA_VER_ID, ver);
    if (ret < 0) {
...
static bool fts_param_need_upgrade(struct i2c_client *client)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_REG_IDE_PARA_STATUS, &val);
    if (ret < 0) {
...
static int fts_fwupg_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_REG_FW_VER, ver);
    if (ret < 0) {
...
int fts_fwupg_get_vendorid(struct fts_ts_data *ts_data, u16 *vid)
{
    ...
    fwvalid = fts_fwupg_check_fw_valid(client);
    if (fwvalid) {
        ret = fts_i2c_read_reg_8719(client, FTS_REG_VENDOR_ID, &vendor_id);
        if (ts_data->ic_info.is_incell)
            ret = fts_i2c_read_reg_8719(client, FTS_REG_MODULE_ID, &module_id);
    } else {
...
int fts_get_firmware_name(char *fw_name,size_t size)
{
    ...
    ret = snprintf(fw_name, size, "TP_Firmware_FT8719.bin");
    if (ret >= size) {
...
int fts_get_pram_name(char *pram_name,size_t size)
{
    int ret = 0;
    
    ret = snprintf(pram_name, size, "touch_8719_pram.img");
    if (ret >= size) {
        FTS_ERROR("%s:pram name buffer out of range, ret=%d\n",
                   __func__, ret);
        return -ENOMEM;
    }
    FTS_INFO("%s:pram name:%s\n", __func__, pram_name);
