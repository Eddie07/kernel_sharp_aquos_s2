...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_i2c_read_reg_8719(client, FTS_REG_ESD_SATURATE, &val);
    if ( ret < 0) {
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret = fts_i2c_read_8719(client, &reg_addr, 1, &reg_value, 1);
        if (ret < 0) {
...
static void esdcheck_func(struct work_struct *work)
{
    ...
    if (ENABLE == fts_esdcheck_data.mode) {
        if (ts_data->ic_info.is_incell) {
            fts_i2c_read_reg_8719(ts_data->client, FTS_REG_ESDCHECK_DISABLE, &val);
            if (0xA5 == val) {
