...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        writelen = 1;
        ret = fts_i2c_write_8719(client, writebuf + 1, writelen);
        if (ret < 0) {
...
/************************************************************************
* Name: fts_create_apk_debug_channel_8719
* Brief:  create apk debug channel
* Input: i2c info
* Output:
* Return: return 0 if success
***********************************************************************/
int fts_create_apk_debug_channel_8719(struct fts_ts_data *ts_data)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
/************************************************************************
* Name: fts_release_apk_debug_channel_8719
* Brief:  release apk debug channel
* Input:
* Output:
* Return:
***********************************************************************/
void fts_release_apk_debug_channel_8719(struct fts_ts_data *ts_data)
{
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts_i2c_read_reg_8719(client, reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts_i2c_read_8719(client, &reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts_i2c_write_reg_8719(client, reg, val);
            } else {
                rw_op.res = fts_i2c_write_8719(client, rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
...
static ssize_t fts_dumpreg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_i2c_read_reg_8719(client, FTS_REG_FW_VER, &val);
    count += snprintf(buf + count, PAGE_SIZE, "FW Ver:0x%02x\n", val);

    fts_i2c_read_reg_8719(client, FTS_REG_LIC_VER, &val);
    count += snprintf(buf + count, PAGE_SIZE, "LCD Initcode Ver:0x%02x\n", val);

    fts_i2c_read_reg_8719(client, FTS_REG_IDE_PARA_VER_ID, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Param Ver:0x%02x\n", val);

    fts_i2c_read_reg_8719(client, FTS_REG_IDE_PARA_STATUS, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Param status:0x%02x\n", val);

    fts_i2c_read_reg_8719(client, FTS_REG_VENDOR_ID, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Vendor ID:0x%02x\n", val);

    fts_i2c_read_reg_8719(client, FTS_REG_LCD_BUSY_NUM, &val);
    ...
    fts_i2c_read_reg_8719(client, FTS_REG_CHARGER_MODE_EN, &val);
    count += snprintf(buf + count, PAGE_SIZE, "charge stat:0x%02x\n", val);

    fts_i2c_read_reg_8719(client, FTS_REG_INT_CNT, &val);
...
void touch_tpfwver_read_8719(char *fw_ver)
{
    ssize_t num_read_chars = 0;
    u8 fwver = 0;
...
void touch_tpfwimver_read_8719(char *fw_ver)
{
}

static int fw_upgrade_flag = 0;
int gForceUpgrade_8719 = 0;
void touch_fwupgrade_8719(int input)
{
    char fwname[FILE_NAME_LENGTH];

    memset(fwname, 0, sizeof(fwname));
    strncpy(fwname, fw_app_bin_8719, FILE_NAME_LENGTH);
    ...
    if (input > 1)
    {
        gForceUpgrade_8719 = 1;
    }
    else
    {
        gForceUpgrade_8719 = 0;
    }
...
EXPORT_SYMBOL(gForceUpgrade_8719);

void touch_fwupgrade_read_8719(char *buf)
{
    sprintf(buf, "%d\n", fw_upgrade_flag);
}

void touch_vendor_read_8719(char *buf)
{
    sprintf(buf, "%s\n","FocalTech");
}
