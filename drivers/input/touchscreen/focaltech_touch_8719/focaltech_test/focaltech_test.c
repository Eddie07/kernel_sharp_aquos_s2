...
bool ResultSelfTest_8719 = false; // FIH self test result
/*****************************************************************************
* functions body
*****************************************************************************/
...
void *fts_malloc_8719(size_t size)
{
    return vmalloc(size);
}
 ...
 *******************************************************************/
int fts_test_i2c_read_8719(u8 *writebuf, int writelen, u8 *readbuf, int readlen)
{
    int ret = 0;
#if 1
    if (NULL == fts_data) {
    ...
    ret = fts_i2c_read_8719(fts_data->client, writebuf, writelen, readbuf, readlen);
#else
    ret = fts_i2c_read_8719(writebuf, writelen, readbuf, readlen);
#endif
...
int fts_test_i2c_write_8719(u8 *writebuf, int writelen)
{
    int ret = 0;
#if 1
    if (NULL == fts_data) {
    ...
    ret = fts_i2c_write_8719(fts_data->client, writebuf, writelen);
#else
    ret = fts_i2c_write_8719(writebuf, writelen);
#endif
...
{
    return fts_test_i2c_read_8719(&addr, 1, val, 1);
}
...
int write_reg(u8 addr, u8 val)
{
    ...
    cmd[0] = addr;
    cmd[1] = val;
    ret = fts_test_i2c_write_8719(cmd, 2);
...
/************************************************************************
* Name: fts_i2c_read_8719_write
* Brief:  Write/Read Data by IIC
* Input: writebuf, writelen, readlen
* Output: readbuf
* Return: Comm Code. Code = 0x00 is OK, else fail.
***********************************************************************/
u8 fts_i2c_read_8719_write(unsigned char *writebuf, int  writelen, unsigned char *readbuf, int readlen)
{
    int ret;

    if (readlen > 0) {
        ret = fts_test_i2c_read_8719(writebuf, writelen, readbuf, readlen);
    } else {
        ret = fts_test_i2c_write_8719(writebuf, writelen);
    }
...
int read_mass_data(u8 addr, int byte_num, int *buf)
{
    ...
    data = (u8 *)fts_malloc_8719(byte_num * sizeof(u8));
    if (NULL == data) {
    ...
    reg_addr = addr;
    ret = fts_test_i2c_read_8719(&reg_addr, 1, data, read_num);
    if (ret) {
    ...
    for (i = 1; i < packet_num; i++) {
        ...
        ret = fts_test_i2c_read_8719(NULL, 0, data + offset, read_num);
        if (ret) {
...
int get_cb_incell(u16 saddr, int byte_num, int *cb_buf)
{
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_test_i2c_read_8719(&cb_addr, 1, data + offset, read_num);
        if (ret) {
...
/*
 * fts_ic_table_get_ic_code_from_ic_name_8719 - get ic code from ic name
 */
u32 fts_ic_table_get_ic_code_from_ic_name_8719(char *ic_name)
{
    int i = 0;
    int type_size = 0;
...
{
    test_data.testresult = fts_malloc_8719(BUFF_LEN_TESTRESULT_BUFFER);
    if (NULL == test_data.testresult)
        goto ERR;

    test_data.store_all_data = fts_malloc_8719(FTS_TEST_STORE_DATA_SIZE);
    if (NULL == test_data.store_all_data)
        goto ERR;

    test_data.store_msg_area = fts_malloc_8719(BUFF_LEN_STORE_MSG_AREA);
    if (NULL == test_data.store_msg_area)
        goto ERR;

    test_data.msg_area_line2 = fts_malloc_8719(BUFF_LEN_MSG_AREA_LINE2);
    if (NULL == test_data.msg_area_line2)
        goto ERR;

    test_data.store_data_area = fts_malloc_8719(BUFF_LEN_STORE_DATA_AREA);
    if (NULL == test_data.store_data_area)
        goto ERR;

    test_data.tmp_buffer = fts_malloc_8719(BUFF_LEN_TMP_BUFFER);
...
static int fts_test_main_init(void)
{
    ...
    ret = malloc_struct_DetailThreshold_8719();
    if (ret < 0) {
    ...
    test_data.buffer = (int *)fts_malloc_8719(len * sizeof(int));
...
static int fts_test_main_exit(void)
{
    ...
    /* free memory */
    free_struct_DetailThreshold_8719();
 ...
 *
 */
static int fts_test_entry_8719(char *ini_file_name)
{
    int ret = 0;
    ...
    if (true == fts_test_start()) {
		ResultSelfTest_8719 = true;
        FTS_TEST_SAVE_INFO("\n=======Tp test pass. \n\n");
    } else {
		ResultSelfTest_8719 = false;
        FTS_TEST_SAVE_INFO("\n=======Tp test failure. \n\n");
    }
...
void touch_selftest_8719(void)
{
    char fwname[128];
    struct i2c_client *client = fts_i2c_client_8719;

    FTS_TEST_FUNC_ENTER();

    memset(fwname, 0, sizeof(fwname));
    strncpy(fwname, tp_ini_8719, 128);
    FTS_TEST_DBG("fwname:%s.", fwname);
...
int selftest_result_read_8719(void)
{
    int num_read_chars = 0;
    if(ResultSelfTest_8719)
        num_read_chars = 0;
    else
        num_read_chars = 1;
