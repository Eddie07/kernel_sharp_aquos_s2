...
//nodes heineken
int swipe_up_enable = 0;
int swipe_down_enable = 0;
int swipe_left_enable = 0;
int swipe_right_enable = 0;
int letter_c_enable =0;
int glove_enable = 0;
int gdouble_tap_enable = 0;
int proximity_enable= 0;
extern int glove_enable;
extern int gdouble_tap_enable;
extern int swipe_up_enable;
extern int swipe_down_enable;
extern int swipe_left_enable;
extern int swipe_right_enable;
extern int letter_c_enable;
extern int proximity_enable;


static struct proc_dir_entry *prEntry_tp = NULL;
static struct proc_dir_entry *prEntry_tp1 = NULL;
...
TS_ENABLE_FOPS(swipe_up);
TS_ENABLE_FOPS(swipe_down);
TS_ENABLE_FOPS(swipe_right);
TS_ENABLE_FOPS(swipe_left);
TS_ENABLE_FOPS(proximity);
/*****************************************************************************
...
//	struct proc_dir_entry *prEntry_tmp  = NULL;
	prEntry_tp1 = proc_mkdir("AllHWList", NULL);
	if( prEntry_tp == NULL ){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create touchpanel\n");
}
	prEntry_tmp = proc_create("tp_double_tap", 0666, prEntry_tp1, &gdouble_tap_proc_fops);
	if(prEntry_tmp == NULL){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create gdouble_tap_enable");
}
	...
	prEntry_tmp = proc_create("letter_c_enable", 0666, prEntry_tp, &letter_c_proc_fops);
	if(prEntry_tmp == NULL){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create letter_c_enable");
}

	prEntry_tmp = proc_create("swipe_up_enable", 0666, prEntry_tp, &swipe_up_proc_fops);
	if(prEntry_tmp == NULL){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create swipe_up_enable");
}
	prEntry_tmp = proc_create("swipe_down_enable", 0666, prEntry_tp, &swipe_down_proc_fops);
	if(prEntry_tmp == NULL){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create swipe_down_enable");
}
	prEntry_tmp = proc_create("swipe_left_enable", 0666, prEntry_tp, &swipe_left_proc_fops);
	if(prEntry_tmp == NULL){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create swipe_left_enable");
}
	prEntry_tmp = proc_create("swipe_right_enable", 0666, prEntry_tp, &swipe_right_proc_fops);
	if(prEntry_tmp == NULL){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create swipe_right_enable");
}

	prEntry_tmp = proc_create("proximity_enable", 0666, prEntry_tp, &proximity_proc_fops);
	if(prEntry_tmp == NULL){
	//	ret = -ENOMEM;
		FTS_ERROR("Couldn't create swipe_up_enable");
}
