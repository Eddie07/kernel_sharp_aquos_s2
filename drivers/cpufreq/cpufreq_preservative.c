...
// WJH static inline cputime64_t get_cpu_idle_time(unsigned int cpu, cputime64_t *wall)
// WJH {
// WJH 	u64 idle_time = get_cpu_idle_time_us(cpu, NULL);
// WJH 
// WJH 	if (idle_time == -1ULL)
// WJH 		return get_cpu_idle_time_jiffy(cpu, wall);
// WJH 	else
// WJH 		idle_time += get_cpu_iowait_time_us(cpu, wall);
// WJH 
// WJH 	return idle_time;
// WJH }
...
static int get_load(struct cpufreq_policy *policy)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		// WJH
		cur_idle_time = get_cpu_idle_time_jiffy(j, &cur_wall_time);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	dbs_info->enable = 1;
	INIT_DELAYED_WORK_ONSTACK(&dbs_info->work, do_dbs_timer); // WJH
	// WJH INIT_DELAYED_WORK_DEFERRABLE(&dbs_info->work, do_dbs_timer);
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			// WJH
			j_dbs_info->prev_cpu_idle = get_cpu_idle_time_jiffy(j,
